...
class PortalSystem {
	...
	init() {
		const fontUrl = '/Barlow Condensed ExtraBold_Regular.json';
		...
		const panoField = this.textureLoader.load('/pano-field.jpg');
		...
		const panoSpace = this.textureLoader.load('/pano-space.jpg');
...
const createThreeScene = (container, uiCallbacks) => {
	...
	const concrete2Tex = textureLoader.load('/concrete-2.jpg');
	...
	textureLoader.load('/nyc.jpg', (tex) => {
				...
				<div className="flex flex-col gap-5">
					{['Get a project quote', 'Partner with Box', 'Employment', 'other'].map((opt) => (
						<label key={opt} className="flex items-center gap-4 text-white/90 text-2xl cursor-pointer group">
							<div className={`w-7 h-7 rounded-full flex items-center justify-center transition-colors ${formData.reason === opt ? 'border-white bg-white/20' : 'border-white/40 group-hover:border-white/70'}`} style={{ borderWidth: '3px', borderStyle: 'solid' }}>
...
const InteractiveLogo = () => {
	...
	return (
		<div className="relative pointer-events-none inline-block" style={{ zIndex: 200 }}>
...
export default function App() {
	...
	useEffect(() => {
		...
		if (!overlay || !viewport || !content) return;
		...
		// Attach wheel/touch to the full-screen overlay so events are always captured
		const handleWheel = (e) => {
			e.preventDefault();
			e.stopPropagation();
	...
	return (
		<div className="m-0 overflow-hidden bg-black font-spirit font-light not-italic w-full relative" style={{ height: windowHeight }}>
			<style dangerouslySetInnerHTML={{__html: `
				...
				/* Responsive overrides for arbitrary values Tailwind v4 can't detect */
				@media (min-width: 768px) {
					.header-bar { bottom: 40px !important; }
					.logo-wrap { width: 125px !important; }
					.nav-links { width: calc(100% - 125px) !important; }
					.scroll-indicator { bottom: 80px !important; }
					.address-col { padding-top: 3.5rem; }
				}
			`}} />

			{/* Loading Overlay */}
			<div className={`absolute inset-0 bg-black flex justify-center items-center transition-opacity duration-1000 ease-out ${loading ? 'opacity-100' : 'opacity-0 pointer-events-none'}`} style={{ zIndex: 100 }}>
				<div className="animate-spin inline-block w-12 h-12 border-current border-t-transparent text-white rounded-full" role="status" style={{ borderWidth: '3px', borderStyle: 'solid' }}>
					<span className="sr-only">Loading...</span>
				</div>
			</div>

			{/* Bottom Center Text */}
			<div className={`absolute flex items-center justify-center text-white border border-white bg-white/10 backdrop-blur-md px-4 py-2 rounded-md pointer-events-none transition-all duration-500 ease-in-out bottom-4 left-1/2 -translate-x-1/2 ${scrollVisible ? 'opacity-100' : 'opacity-0'}`} style={{ zIndex: 50 }}>
				<span className="block font-spirit tracking-normal normal-case leading-none text-center whitespace-nowrap" style={{ fontSize: '1rem' }}>
					Contact us for work samples. Full site coming soon!
				</span>
			</div>

			{/* Header */}
			<header className={`header-bar absolute left-0 w-full px-4 md:px-8 pointer-events-none flex items-center transition-opacity duration-500 ${uiVisible ? 'opacity-100' : 'opacity-0'}`} style={{ bottom: '60px', zIndex: 25 }}>
				<div className={`logo-wrap shrink-0 ${uiVisible ? 'pointer-events-auto' : 'pointer-events-none'}`} style={{ width: '80px' }}>
					<InteractiveLogo />
				</div>
				<div className={`nav-links flex justify-end items-center gap-6 md:gap-10 text-white ${uiVisible ? 'pointer-events-auto' : 'pointer-events-none'}`} style={{ width: 'calc(100% - 90px)' }}>
					<span
						className="inline-block font-spirit text-right transition-all duration-300 ease-in-out relative cursor-pointer opacity-80 hover:opacity-100 hover:-translate-y-0.5"
			...
			{/* Intro SVG Graphic */}
			<div className={`absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full pointer-events-none transition-opacity duration-1000 ease-in-out ${!loading && introVisible && scrollMode === 'orbit' ? 'opacity-100' : 'opacity-0'}`} style={{ zIndex: 30 }}>
				<img 
					src="/intro.svg" 
					alt="Box Creative" 
			...
			{/* Scroll Indicator */}
			<div className={`scroll-indicator absolute flex flex-col items-center text-white pointer-events-none transition-all duration-500 ease-in-out border-2 border-white overflow-hidden box-border left-1/2 -translate-x-1/2 ${scrollVisible ? 'opacity-100' : 'opacity-0'}`} style={{ zIndex: 40, height: '40px', paddingLeft: '15px', paddingRight: '15px', bottom: '180px' }}>
				<div className="flex flex-col animate-infinite-scroll">
					<span className="block font-spirit tracking-normal normal-case text-center" style={{ fontSize: '1.2rem', height: '36px', lineHeight: '36px' }}>Scroll</span>
					<span className="block font-spirit tracking-normal normal-case text-center" style={{ fontSize: '1.2rem', height: '36px', lineHeight: '36px' }}>Scroll</span>
				</div>
			</div>
			...
			<div ref={portalOverlayRef} className={`absolute inset-0 bg-black/60 transition-opacity duration-1000 ease-in-out ${portalUiVisible ? 'opacity-100' : 'opacity-0'}`} style={{ zIndex: 30, pointerEvents: portalUiVisible ? 'auto' : 'none' }}>
				...
				>
					...
					<div className="absolute right-2 top-2 bottom-2 w-1.5 bg-white/10 rounded-full pointer-events-none" style={{ zIndex: 50 }}>
					...
					>
						{activePortalId === 'portal3' ? (
							<div className="text-white font-spirit leading-normal" style={{ maxWidth: '100rem' }}>
							...
							<div className="text-white font-spirit leading-normal" style={{ maxWidth: '100rem' }}>
								...
								<div className="grid grid-cols-1 md:grid-cols-2 gap-10 max-w-4xl mb-12 opacity-90 text-2xl md:text-3xl font-light">
									...
									<div className="flex flex-col gap-1 address-col">
								...
								<div className="mb-24 w-full max-w-4xl bg-white/5 p-8 md:p-12 rounded-2xl backdrop-blur-xl border border-white/20 relative" style={{ zIndex: 50 }}>
									<ContactForm />
								</div>
							</div>
						) : null}
			...
			<button 
				...
				className={`absolute bottom-0 right-0 m-4 bg-white/40 border border-black rounded-full text-black p-0 justify-center items-center transition-all duration-300 ease-in-out cursor-pointer hover:bg-white/90 hover:scale-105 pointer-events-auto ${backVisible ? 'animate-fade-in flex' : 'hidden'}`}
				style={{ boxShadow: '0 4px 12px rgba(0,0,0,0.1)', backdropFilter: 'blur(12px)', width: '50px', height: '50px', zIndex: 50 }}
			>
				<svg xmlns="http://www.w3.org/2000/svg" fill="currentColor" className="w-5 h-5" style={{ strokeWidth: '1px' }} viewBox="0 0 16 16">
